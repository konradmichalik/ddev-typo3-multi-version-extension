...
- [ ] The classic-mode `post-install` hook runs with `$BASE_PATH` (not `$BASE_PATH/public`) as its working directory, matching Composer mode (CodeRabbit review on PR #64) - not yet exercised live
...
- [ ] `worktree-init` fails explicitly (instead of guessing `12 13 14`) when `TYPO3_VERSIONS` can't be read from either `docker-compose.zz-project.yaml` or `docker-compose.typo3-setup.yaml` (issue #63 item 6) - not yet exercised live
- [ ] `worktree-init` picks up a `TYPO3_VERSIONS` override from `docker-compose.zz-project.yaml`, consistent with the `config.typo3-setup.yaml` hostnames generated by `ddev add-on get` (KYD follow-up to issue #63 items 11/12 - the two lookups had drifted out of sync) - not yet exercised live
- [ ] A `docker-compose.zz-project.yaml` override for `TYPO3_VERSIONS` actually wins over `docker-compose.typo3-setup.yaml` (CodeRabbit review on PR #64 - the original `docker-compose.project.yaml` name sorted *before* the managed file alphabetically, so the add-on's value silently won instead of the override) - not yet exercised live
...
- [ ] `.ddev/docker-compose.zz-project.yaml` (copied from `docker-compose.zz-project.yaml.example`) overrides an add-on-managed env var (e.g. `TYPO3_CONTEXT`) and survives `ddev add-on get` (issue #63 item 12) - not yet exercised live
- [ ] Overriding `TYPO3_VERSIONS` via `docker-compose.zz-project.yaml` survives `ddev add-on get`, and the regenerated `config.typo3-setup.yaml`'s `additional_hostnames` matches it instead of the hardcoded `11-14` (issue #63 item 11) - not yet exercised live
